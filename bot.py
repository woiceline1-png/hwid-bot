...
async def init_db():
    async with aiosqlite.connect(DATABASE_FILE) as db:
        # --- Cek apakah tabel users pake schema LAMA (hwid UNIQUE) ---
        needs_rebuild = False
        try:
        ...
        if needs_rebuild:
            print("🔄 Migrasi: rebuild tabel users tanpa UNIQUE pada hwid...")
            try:
                await db.execute('''
                ...
                print(f"⚠️ Migrasi gagal: {e}")
        else:
            # Buat tabel baru (tanpa UNIQUE) kalau belum ada
            await db.execute('''
        ...
        # Tambah kolom expiry_date kalau belum ada
        try:
...
async def check_hwid(ctx, user_input: str = None):
    ...
    async with aiosqlite.connect(DATABASE_FILE) as db:
        ...
        if row:
            ...
            embed.add_field(name="HWID", value=f"`{hwid}`" if hwid else "None", inline=False)
            ...
            status = "⏰ EXPIRED" if expired else ("🟢 Active" if verified else "⛔ Unverified")
...
async def verify_hwid(ctx, user_input: str, hwid: str, expiry_days: int = 30):
    ...
    async with aiosqlite.connect(DATABASE_FILE) as db:
        # Cek apakah user ini SUDAH VERIFIED
        cursor = await db.execute(
        ...
        # Cek HWID bentrok HANYA pada user yang masih VERIFIED
        cursor = await db.execute(
            'SELECT discord_id FROM users WHERE hwid = ? AND discord_id != ? AND verified = 1',
            (hwid, user.id)
        )
        ...
        # Bersihkan HWID dari user UNVERIFIED yang dulu punya HWID ini
        await db.execute(
            'UPDATE users SET hwid = NULL WHERE hwid = ? AND verified = 0',
            (hwid,)
        )
...
async def unverify_hwid(ctx, user_input: str):
    ...
    async with aiosqlite.connect(DATABASE_FILE) as db:
        # Set verified=0 DAN hapus HWID agar bisa dipakai user lain
        await db.execute(
            'UPDATE users SET verified = 0, hwid = NULL WHERE discord_id = ?',
            (user.id,)
        )
        await db.commit()
    ...
    await ctx.send(f"✅ HWID unverified for {display_name}! HWID sekarang tersedia untuk dipakai ulang.")
...
async def list_hwid(ctx):
    async with aiosqlite.connect(DATABASE_FILE) as db:
        ...
        for row in rows[:10]:
            ...
            hwid_display = f"`{hwid[:8]}...`" if hwid else "None"
            embed.add_field(
                name=f"<@{discord_id}>",
                value=f"HWID: {hwid_display}\nExpiry: {exp_display}\nStatus: {status}",
                inline=False
            )
        await ctx.send(embed=embed)
